  ...
  root.innerHTML = `
    <div class="device-frame" data-player-open="false">
      ...
      <aside class="account-sheet" id="accountSheet" aria-hidden="true" aria-labelledby="accountSheetTitle" hidden>
        ...
        <section class="account-own-account-card">
          <div><strong>Want your own account?</strong><span>This web player signs in as one shared YouTube account — whoever set up the server. For your own library and recommendations, install the OpenTune Android app and sign in there on your own phone.</span></div>
          <a class="account-primary" id="androidAppLink" href="https://github.com/Arturo254/OpenTune/releases" target="_blank" rel="noopener noreferrer">Get the Android app</a>
        </section>
        ...
        <section class="account-extension-card" id="extensionLoginCard"><div><strong>Android-style Google login</strong><span id="extensionLoginText">Use the OpenTune Login Helper extension to open Google login and capture the YouTube Music session automatically.</span></div><button class="account-primary" id="extensionLoginButton" type="button">Login with YouTube Music</button></section>
        <section class="account-server-login-card" id="serverLoginCard" hidden><div><strong>Sign in on the server machine</strong><span id="serverLoginText">The browser helper can only capture a login on the computer running the server. Open OpenTune there at its localhost address to sign in, or use Pair with OpenTune Android above.</span></div></section>
